...
# Problem 13.3
```{r}
bond_data <- read.csv("Ch13.Ex3.JamesBondMoviesData/BondUpdate.csv")
```

## Problem A
```{r}
ThreeA <- lm(GrossRev~ Rating + Budget, data=bond_data)
summary(ThreeA)
```
...
summary(AuxReg)
```
Since the coefficient on LagErrors is statistically significant, we can say that there is autocorrelation in our errors.

## Problem B
```{r}
# referenced code from Bailey's Chapter 13 computing corner
Rho_hat <- summary(AuxReg)$coefficients[2]
bond_data$LagGrossRev <- c(NA, bond_data$GrossRev[1:nrow(bond_data) -1])
bond_data$LagRating <- c(NA, bond_data$Rating[1:nrow(bond_data) -1])
bond_data$LagBudget <- c(NA, bond_data$Budget[1:nrow(bond_data) -1])

bond_data$RhoGrossRev <- bond_data$GrossRev - Rho_hat*bond_data$LagGrossRev
bond_data$RhoRating <- bond_data$Rating - Rho_hat*bond_data$LagRating
bond_data$RhoBudget <- bond_data$Budget - Rho_hat*bond_data$LagBudget

summary(lm(RhoGrossRev ~ RhoRating + RhoBudget, data=bond_data))
```
The coefficients on Rating and Budget are a bit higher than the corresponding coefficient in the first regression. Bailey notes that it is ok that the coefficient in the rho transformed model is different than the coefficient in the original OLS (both are still not subject to bias in OLS because they are both estimates of the true beta). The standard error on the Budget coefficient is larger in the part b regression than in the part a regression which is to be expected in the autocorrelation corrected model vs the non-corrected model. I would not have expected the Rating standard error to be smaller, but it is. The error term is no longer autocorrelated (the autocorrelation did go away).

## Problem C
```{r}
# referenced code from Bailey's Chapter 13 computing corner
ThreeC <- lm(GrossRev ~ lag(GrossRev) + Rating + Budget, data=bond_data)
summary(ThreeC)
```
In the short term, a one unit increase in rating will increase GrossRev by 190.75. In the long run, a one unit increase in Rating will lead to a 439.219 unit increase in Gross Rev (calculation below).
...
## Problem D
```{r}
bond_data$ChangeRev <- bond_data$GrossRev - bond_data$LagGrossRev
summary(lm(ChangeRev ~ lag(GrossRev) + Rating + Budget + lag(ChangeRev), data=bond_data))
```
Using the significance values from p. 473 of Bailey, GrossRev, Rating, and budget are all subject to nonstationarity.

## Problem E
```{r}
# differenced
summary(lm(I(GrossRev - lag(GrossRev)) ~ I(lag(GrossRev)-(lag(lag(GrossRev)))) + I(Rating - lag(Rating)) + I(Budget - lag(Budget)), data=bond_data))
```
The model now uses differenced independent and dependent variables, so we have one fewer data points with which to work with. For each one unit increase in first difference of Rating, there is a 201.13 unit increase in the first difference of GrossRev.

## Problem F
```{r}
summary(lm(GrossRev ~ Rating + Budget + Actor, data=bond_data))
```
Including actor in the regression indicates that Sean Connery has the largest effect on revenue.
...
summary(RhoHat)
```
The coefficient on LagResiduals is highly statistically significant, so there does appear to be autocorrelation. Autocorrelation will not bias our coefficients but does mean that we are underestimating our standard errors.
...
```{r}
library(plm)
residuals <- as.numeric(names((plm(medals~lag(medals) + population + GDP + host, data=olympicsdata, index=c("year", "country"), effect="twoways")$residuals)))
lagresiduals <- c(NA, residuals[1:length(residuals)-1])
RhoHat<-lm(residuals~lagresiduals)
summary(RhoHat)
```
There is! Just as in Part C, there is still autocorrelation. Bailey notes that dynamic models that do not include fixed effets are biased if the errors are correlated. I believe that this also applies to fixed effects, dynamic models. Since parts c and f show that errors are correlated, we may not want to use dynamic models here. Section 13.5 of Real Stats discusses how autocorrelated errors will cause bias in a dynamic model, while it will not in OLS, making the OLS model seem more attractive in this case.
