...
```
The coefficient on lowtemp_rho is a bit lower than the corresponding coefficient in the first regression. Baily notes that it is ok that the coefficient in the rho transformed model is different than the coefficient in the original OLS (both are still not subject to bias). The standard error on the low temperature coefficient is larger in the part b regression than in the part a regression (24.85 vs 22.78, respectively).
...
olympicsdata$LagResiduals[LagID != ID] = NA
olympicsdata$LagResiduals[olympicsdata$LagID != ID] = NA
...
## Prolem D
```{r}
# Used code from Bailey's Chapter 15 compute corner
...
population_rho <- (olympicsdata$population[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$population[olympicsdata$LagID==olympicsdata$ID])
gdp_rho <- (olympicsdata$GDP[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$GDP[olympicsdata$LagID==olympicsdata$ID])
host_rho <- (olympicsdata$host[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$host[olympicsdata$LagID==olympicsdata$ID])
...
gdp_rho <- (olympicsdata$GDP[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$GDP[olympicsdata$LagID==olympicsdata$ID])
host_rho <- (olympicsdata$host[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$host[olympicsdata$LagID==olympicsdata$ID])
lag_medals_rho <- (olympicsdata$LagMedals[olympicsdata$LagID==olympicsdata$ID] - RhoHat$coefficients[2]*olympicsdata$LagMedals[olympicsdata$LagID==olympicsdata$ID])
countries <- (olympicsdata$country[olympicsdata$LagID==olympicsdata$ID])
years <- (olympicsdata$year[olympicsdata$LagID==olympicsdata$ID])

summary(lm(medals_rho~population_rho + gdp_rho + host_rho + lag_medals_rho + factor(countries) + factor(years)))
```
The coefficients on host and population are much closer betwen part g and d than part g and part e. The coefficient on gdp is pretty similar in all three cases.
...
## Probem I
I think the dynamic model is more useful in this case. I would expect that the number of medals earned by a country in the last olympics is highly indicative of how many they will win the next time for myriad reasons. If we had more variables to include on the right side of the equation, perhapds we would not need to use a dynamic model, but in absence of a lot of omitted variables, I think the dynamic model is something that we should strive to estimate. Furthermore, even though we're worried about bias, we have 10 data points per country, which is not negligible, so perhaps bias isn't a huge issue for us in this case.

## Problem J
The models with more data (i.e. those that do not include logged variables) are likely to be more robust in my opinion. That's because each individual data point should hold less weight, all else being equal, if it is in a dataset that has many points as opposed to fewer points.
